...
export const ImportWorkbench: React.FC = () => {
  ...
  // Find default Import contract
  const activeContract = useMemo(() => {
    const found = contracts.find(
      (c) => c.direction === 'IMPORT' && c.contractNumber === 'MHI-IMP-001'
    );
    if (found) return found;
    const fallback = contracts.find((c) => c.direction === 'IMPORT');
  ...
  // Keep local state in sync ONLY if contract ID changes
  useEffect(() => {
  ...
  const runValidation = useMemo(() => {
    ...
    const missingDrop = localContract.routes.some((r) => !r.dropLocationName || !r.dropLocationCode);
    if (missingDrop) {
      return { canRun: false, reason: 'Every active route must have a valid Drop Location.' };
    }
  ...
  // Handle Lump Sum mode selection
  ...
  // Update 20s Slab Column Header Captions button click
  ...
  // Update 40s Slab Column Header Captions button click
  ...
  const handleRunGeneration = async () => {
    ...
    try {
      ...
      setGenerationSuccess(
        `IMPORT RUN completed: Generated ${run.mainRecordsCount} haulage records and ${run.weightSlabRecordsCount} child Weight Slab rows (IDs starting at ${run.records[0]?.id || 1001}).`
      );
      addAuditLog({
        user: 'system.operator@haulage.intelligence',
        ...
        entity: 'Contract',
        entityId: localContract.contractNumber,
        summary: `Executed deterministic Import haulage run for ${localContract.contractNumber} (${run.mainRecordsCount} records).`,
      });
    } catch (err: any) {
  ...
  return (
    <div className="p-5 max-w-[1720px] mx-auto space-y-4 text-slate-800">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-2 border-b border-slate-200">
        <div>
          <div className="flex items-center space-x-2.5">
            <div className="p-1.5 bg-[#0B1F33] text-white rounded-md">
              <ArrowDownLeft className="w-5 h-5 text-blue-400" />
            </div>
            <div>
              <h1 className="text-lg font-bold text-slate-900 tracking-tight">
                Haulage Input Sheet for Import
              </h1>
              <p className="text-xs text-slate-500 font-mono">
                Original Excel/VBA Interactive Rate Workbench (2015–2016 Reproduction)
              </p>
            </div>
          </div>
        </div>
              ...
              {contracts
                ...
                .map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.contractNumber} — {c.pickupLocationName} ({c.amountType === 'WEIGHT_SLAB' ? 'Weight Slab' : 'Lump Sum'})
                  </option>
                ))}
            </select>
          </div>
      ...
      {/* FEEDBACK BANNERS */}
      {slabUpdateFeedback && (
        <div className="bg-blue-50 border border-blue-300 text-blue-900 px-4 py-2 rounded-lg flex items-center justify-between text-xs animate-in fade-in">
          <div className="flex items-center space-x-2">
      ...
      {/* MAIN ROUTE / RATE ENTRY MATRIX (35-Column Modern Spreadsheet) */}
      <div className="pt-2">
        <HaulageRateGrid
          header={localContract}

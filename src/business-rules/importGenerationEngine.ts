...
export function generateImportHaulageRecords(
  ...
  const portCode = contract.pickupLocationCode;
  ...
  contract.routes.forEach((route) => {
    ...
    // Expand route across each applicable Import PortEquipmentMapping
    activeMappings.forEach((mapping) => {
      ...
      if (contract.amountType === 'LUMPSUM') {
        ...
        // WEIGHT_SLAB: Main haulage amount is 0 per business specification
        recordAmount = 0;
      ...
      // Resolve country codes accurately from Location Master
      ...
      const mainRecord: HaulageMainRecord = {
        ...
        pickupZipDepotTerminal:
          route.pickupFacilityCode || mapping.terminalCode,
        ...
        dropZipDepotTerminal: route.dropFacilityCode || '',
        ...
        amount: recordAmount,
        portToPay: route.portToPay || contract.portToPay || contract.pickupLocationCode,
        payableAt: route.payableAt || contract.payableAt || 'POD',
        negotiatedOn: route.negotiatedOn || contract.negotiatedOn,
        negotiatedBy: route.negotiatedBy || contract.negotiatedBy,
        returnLocation:
          route.returnLocationName || contract.returnLocationName || contract.pickupLocationName,
        returnType: route.returnType || contract.returnType,
        returnZipDepotTerminal: route.returnFacilityCode || '',
        validFrom: route.validFrom || contract.validFrom,
        validTo: route.validTo || contract.validTo,
        pickupCountryCode: pickupLoc ? pickupLoc.countryCode : 'DE',
        viaHubLocationCode: mapping.compositePortEquipmentCode, // e.g. DEHAM20 / DEHAM40
        dropCountryCode: dropLoc ? dropLoc.countryCode : 'CZ',
        ...
        _trace: {
          ...
          generatedBy,
          explanationText: `Import route to ${route.dropLocationName} expanded for ${mapping.terminalName} (${mapping.terminalCode}) in ${eqSize}. Pricing applied from ${sourceAmountField} (${contract.amountType}).`,
        },
      };
      ...
      if (contract.amountType === 'WEIGHT_SLAB') {
        ...
        // Count non-generated zero slabs
...
export const generateImportTrustRecords = generateImportHaulageRecords;
